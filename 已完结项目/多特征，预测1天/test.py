...
for code in code_list:
    ...
    try:
        ...
        turn_list = list(result.loc[:,'turn'])
        ...
        sample = np.array([open_datas,high_datas,low_datas,close_datas,volume_datas,turn_datas])
        ...
        for i in range(len(result)-HISTORY-pre_num-1):
            ...
            if (float(result.iloc[n+1,5])-float(result.iloc[n,5]))*(possibility[i]-float(result.iloc[n,5])) < 0 :
                incorrect +=1
        bucket.append((code,result.iloc[-1,5],possibility[-2],(float(result.iloc[-1,5])/float(result.iloc[-1-pre_num,5]))-1,(possibility[-2]/possibility[-3])-1))
        print('success:' + code)
    except:
        pass
