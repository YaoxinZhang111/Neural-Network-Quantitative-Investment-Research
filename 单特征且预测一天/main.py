...
daily_price = pd.read_csv("sz_50.csv", parse_dates=['date'])
daily_price.close = scaler.fit_transform(daily_price.iloc[:,5:6].values)
...
for stock in daily_price['code'].unique():
    ...
    # 导入数据
    datafeed = bt.feeds.PandasData(dataname=data_, fromdate=datetime.datetime(2010, 1, 1),
    ...
    def next(self):
        ...
        try:
            ...
            for d, i in enumerate(self.datas):
                ...
                sample = np.reshape(np.array(sample), (1, HISTORY, 1))

    
                possibility = model.predict(sample)
                possibility = scaler.inverse_transform(possibility)
                possibility = scaler.inverse_transform(possibility)
                possibility = (possibility/i.lines.close[-1])-1
